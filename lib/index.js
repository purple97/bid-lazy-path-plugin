...
var BidLazyPathPlugin = /*#__PURE__*/function () {
  ...
  _createClass(BidLazyPathPlugin, [{
    ...
    value: function recursionReplacePath(compilation, version) {
      ...
      for (var _i3 = 0, _Object$keys3 = Object.keys(newAssets); _i3 < _Object$keys3.length; _i3++) {
        var _name = _Object$keys3[_i3];

        if (/\.(js|jsx|ts|tsx)$/.test(_name)) {
          if (_name.indexOf(version) == -1) {
            var dirpath = mainDir ? mainDir + '/' : '';
            var newName = this.createNewPath(dirpath + _name, version);
            newAssets[newName] = newAssets[_name];
            delete newAssets[_name];
            this.setAssetsInfoByDependency(newAssetsInfo, _name, newName);
          }
        }
      }
